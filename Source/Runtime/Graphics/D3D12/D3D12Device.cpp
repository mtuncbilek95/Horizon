...
#include <Runtime/Graphics/D3D12/D3D12Buffer.h>
...
#include <Runtime/Graphics/D3D12/D3D12Queue.h>
...
namespace Horizon
{
	...
	GfxPointer<GfxTexture> D3D12Device::CreateTexture(const GfxTextureDesc& desc)
	{
		...
		return GfxPointer<GfxTexture>(tex);
	}

	GfxPointer<GfxBuffer> D3D12Device::CreateBuffer(const GfxBufferDesc& desc)
	{
		...
		rd.Flags = has(desc.usage, GfxBufferUsage::Storage)
		...
		if (FAILED(hr)) { delete buf; return {}; }
		...
		return GfxPointer<GfxBuffer>(buf);
	}

	std::unique_ptr<GfxQueue> D3D12Device::CreateQueue(GfxQueueType type)
	{
		auto queue = std::make_unique<D3D12Queue>();
		queue->m_type = type;
	...
	std::unique_ptr<GfxFence> D3D12Device::CreateFence()
	{
	...
	std::unique_ptr<GfxCommandList> D3D12Device::CreateCommandList(GfxQueueType type)
	{
	...
	std::unique_ptr<GfxSwapchain> D3D12Device::CreateSwapchain(const GfxSwapchainDesc& desc, GfxQueue* presentQueue)
	{
	...
	void D3D12Device::CreateTerminalLog()
	{
		...
		auto cb = [](D3D12_MESSAGE_CATEGORY, D3D12_MESSAGE_SEVERITY sev,
			...
			{
				MainLog::Terminal("[D3D12][{}]: {}", u32(sev), desc);
			};
