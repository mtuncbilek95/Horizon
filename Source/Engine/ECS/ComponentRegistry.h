...
namespace Horizon
{
	...
	{
	public:
		ComponentArray() { m_sparse.fill(~0u); }
		~ComponentArray() = default;

		void Insert(EntityHandle entity, T component)
		{
			assert(m_sparse[entity.id] == ~0u && "Already registered entity for the component");
		...
		{
			assert(m_sparse[entity.id] != ~0u && "Invalid entity");
		...
		void Remove(EntityHandle entity)
		{
			...
			assert(m_sparse[entity.id] != ~0u && "Entity does not have this component");
		...
		b8 HasEntity(EntityHandle entity) const { return m_sparse[entity.id] != ~0u; }
